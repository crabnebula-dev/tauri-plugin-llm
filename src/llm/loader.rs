...
mod tests {
    ...
    // fn load_sanitize_json<T: DeserializeOwned>(input: &PathBuf) -> Result<T> {
    //     let mut value: serde_json::Value = serde_json::from_slice(&std::fs::read(input)?)?;

    //     if let serde_json::Value::Object(ref mut map) = value {
    //         for (_key, val) in map.iter_mut() {
    //             if val.is_null() {
    //                 *val = serde_json::Value::Number(0.into());
    //             }
    //         }
    //     }
